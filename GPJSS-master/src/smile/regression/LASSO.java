...
public class LASSO  implements Regression<double[]> {
    ...
    public LASSO(double[][] x, double[] y, double lambda, double tol, int maxIter) {
        ...
        center = Math.colMeans(x);//get average value of each column as the center
    ...
    public static void main(String[] args) {
        ...
        //double t = Math.min(Math.max(1.0, 1.0 / lambda), 2 * p / 1e-3);
        LASSO.Trainer trainer = new LASSO.Trainer(1);//when set to 1, that value will be 1; if we want to increase the value, set 1 to 0.3, 0.2 (smaller than 1)
        LASSO network = trainer.train(x, y);

        double[] x_test = new double[]{0.0, 1.1, 2.1};
