...
public class SVR<T> implements Regression<T> {
    ...
    public SVR(T[] x, double[] y, double[] weight, MercerKernel<T> kernel, double eps, double C, double tol) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            v.g[0] = eps + y[i];
            v.g[1] = eps - y[i];
            v.k = kernel.k(x[i], x[i]); //always equals 1---e^(-x)
            sv.add(v);
        }
        ...
        while (iter.hasNext()) {
            ...
            if (v.alpha[0] == 0.0 && v.alpha[1] == 0.0) { //when first initialise the sv, these two alpha value are 0
                iter.remove();
            }
        }
    ...
    public static void main(String[] args){
        ...
        Trainer<double[]> trainer = new Trainer<double[]>(mercerKernel,1,1.0);
        SVR<double[]> network = trainer.train(x, y);
