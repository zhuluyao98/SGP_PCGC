...
public class surrogateClearingMultiPopCoevolutionaryEvaluatorV2 extends MultiPopCoevolutionaryEvaluator {
        ...
        //do not clear
          ...
          /*  int[][] tempIndsCharListsForModelSubPop0 = null;
            ...
            for (int subpop = 0; subpop < state.population.subpops.length; subpop++) {
                ...
                List<Double> tempFitnessesForModel = new ArrayList<>();
                int removeIdx = 0;
                ...
                for (int i = 0; i < state.population.subpops[subpop].individuals.length; i++) {
                    ...
                    if (ind.fitness.fitness() == Double.MAX_VALUE) {
                        ...
                        tempFitnessesForModel.add(ind.fitness.fitness());
                    }
                }

                if(fitnessesForModel.size() == 0 || fitnessesForModel.size() == 1){
                    fitnessesForModel.add(subpop, tempFitnessesForModel.toArray(new Double[tempFitnessesForModel.size()]));
                }
                else{
                    Double[] combineFitness = ArrayUtils.addAll(fitnessesForModel.get(subpop), tempFitnessesForModel.toArray(new Double[tempFitnessesForModel.size()]));
