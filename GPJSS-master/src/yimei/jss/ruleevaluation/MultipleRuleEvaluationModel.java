...
//import yimei.jss.algorithm.featureweighted.backup.multiPopCoevolutionaryEvaluator;
...
public class MultipleRuleEvaluationModel extends AbstractEvaluationModel{
    ...
    ArrayList<Double[]> fitnessFixedSimulation = new ArrayList<>();
    ArrayList<Double> fitnessFixedSimulationSubPop = new ArrayList<>();
    int count = 0;
...
//    protected SchedulingSet schedulingSet;
    public static SchedulingSet schedulingSet;
        ...
        for (int j = 0; j < simulations.size(); j++) {
            ...
            for (int k = 1; k < schedulingSet.getReplications().get(j); k++) {
                ...
                /*    double normObjValue = simulation.objectiveValue(objectives.get(i))
                    ...
                    //fzhang 2019.9.20 keep the fitness for building the surrogate
                    //==========================================start================================================
                 /*   fitnessFixedSimulationSubPop.add(normObjValue);
                    if (fitnessFixedSimulationSubPop.size() % state.population.subpops[0].individuals.length == 0) {
                        if (count % 2 == 0) {
                            subpop = 0;
                        } else {
                            subpop = 1;
                        }
                        if (fitnessFixedSimulation.size() == 0 || fitnessFixedSimulation.size() == 1) {
                            fitnessFixedSimulation.add(subpop, fitnessFixedSimulationSubPop.toArray(new Double[fitnessFixedSimulationSubPop.size()]));
                        } else {
                            Double[] combineFitness = ArrayUtils.addAll(fitnessFixedSimulation.get(subpop), fitnessFixedSimulationSubPop.toArray(new Double[fitnessFixedSimulationSubPop.size()]));
                            fitnessFixedSimulation.set(subpop, combineFitness);
                        }
                        count++;
                        fitnessFixedSimulationSubPop.clear();
                    }*/
                   //==============================end=====================================
      ...
      //modified by fzhang 18.04.2018  in order to check this loop works or not after add filter part: does not work
       ...
       // }
        ...
        for (int i = 0; i < fitnesses.length; i++) {
            ...
            //fzhang 2019.9.20
            //this is for evolving model---surrogate, because sometimes the model will predict negative (unreasonable) results
            //======================================start=========================
          /*  if(fitnesses[i] == 0 || fitnesses[i] == Double.POSITIVE_INFINITY || fitnesses[i] == Double.NEGATIVE_INFINITY){
...
/*    public void WriteCountBadrun(EvolutionState state, final Parameter base) {
	...
	public ArrayList<Double[]> getFitnessFixedSimulation(){
        return fitnessFixedSimulation;
    }

    public void clearingFitnessFixedSimulation(){
        fitnessFixedSimulation.clear();
    }
}
