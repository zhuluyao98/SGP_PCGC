...
//				weights[subpop] = new double[terminals[subpop].length];
		...
		if (generation >= preGenerations)
		{
			...
			//and the other is its frequency
			//stats.toString();
			weights = new double[2][]; //the number of trees for each individual
			for(int numTree = 0; numTree < 2; numTree++)
			{
				weights[numTree] = new double[terminals[0].length];
				for(int i = 0; i < terminals[0].length;i++)
				{
					String name = (terminals[0][i]).name();//the terminals in each population is same.
					for (int indIdx = numTree*topInds; indIdx < numTree*topInds+topInds; indIdx ++) {
						if(stats.get(indIdx).containsKey(name))
						{
							weights[numTree][i] += stats.get(indIdx).get(name);
						}
					}
				}
				//save the weights values in each generation
				//saveWeights.add(weights[subpop]) this is a java style, the weights will be changed later
				saveWeights.add(weights[numTree].clone()); //need to use clone to copy the array
				RandomChoice.organizeDistribution(weights[numTree]); // organizeDistribution will change the input --- weights[subpop], this is what we want. It will be used for pick the terminals
			}
		}
