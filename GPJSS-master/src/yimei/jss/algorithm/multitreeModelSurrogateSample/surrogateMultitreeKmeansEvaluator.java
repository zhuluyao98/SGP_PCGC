...
import yimei.jss.algorithm.multitreeModelSurrogate.Kmeans;
...
public class surrogateMultitreeKmeansEvaluator extends SimpleEvaluator {
...
//            multitreeClearingFirst.clearPopulation(state, radius, capacity, phenoCharacterisation);
            ...
            if (nonIntermediatePop) {
                ...
                Kmeans kmeans = new Kmeans(250);
                ArrayList<double[]> dataSet=new ArrayList<>();
                //把二维数组 int类型转换成 double类型;
                for (int num=0; num<indsCharListsMultiTree.length; num++) {
                    double[] tempp = new double[indsCharListsMultiTree[0].length];
                    for (int i = 0; i < indsCharListsMultiTree[0].length; i++) {
                        tempp[i] = indsCharListsMultiTree[num][i];
                    }
                    dataSet.add(tempp);
                }
                //设置原始数据集
                ...
                kmeans.execute();
                //得到聚类结果
                ArrayList<ArrayList<double[]>> cluster=kmeans.getCluster();
                //查看结果
                ...
                //将已评估的个体挑选出来，建立代理模型
                ArrayList<Integer> indexEvaluated = new ArrayList<>();
                for (int pop=0; pop<state.population.subpops.length; pop++){
                    for (int ind=0; ind<state.population.subpops[pop].individuals.length; ind++){
                        Individual inds = state.population.subpops[pop].individuals[ind];
                        if(inds.evaluated == true){
                            indexEvaluated.add(ind);
                        }
                    }
                }
