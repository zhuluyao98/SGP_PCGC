...
public class surrogateMultitreePCNichingEvaluatorV2 extends SimpleEvaluator {
...
/*                Collections.sort(listPcClusterFit, new Comparator<HashMap.Entry<int[], Integer>>() {
                ...
                if (state.generation == 0) {
                    ...
                    for (int a=0; a<pcIndex1.length; a++){
                        ...
                        double[] indsTerminalFrequencySetBase = getNumTerminalSet(indsBase);


                        for(int b=1; b<pcIndex1[a].length; b++){
                            Individual indsCounter = state.population.subpops[0].individuals[pcIndex1[a][b]];
                            double[] indsTerminalFrequencySetCounter = getNumTerminalSet(indsCounter);
                            double correlation = new SpearmansCorrelation().correlation(indsTerminalFrequencySetBase, indsTerminalFrequencySetCounter);
