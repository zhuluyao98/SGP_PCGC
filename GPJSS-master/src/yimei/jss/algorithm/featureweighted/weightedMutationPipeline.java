...
public class weightedMutationPipeline extends MutationPipeline {
                       ...
                       final int thread)
    {
        ...
        for(int q=start; q < n+start; q++) //q=start = 0  q<1
        {
            ...
            //fzhang 2019.8.29 for mutation, choose the worse part and use a new generated subtrees to replace it.
            //worse part means not important, so the small the value, the more chance to be selected
            for(int x=0;x<numTries;x++)
            {
                // pick a node in individual 1
                p1 = nodeselect.pickNode(state,subpopulation,thread,i,i.trees[t], true); //set to true, choose the function with smaller score and tend to make the individual better
                                                                                         //set to false, choose the function with larger score and tend to make the individual worse/diverse?
                                                                                         //it is hard to say, so, do not apply any strategy on mutation first.
