...
public class weightedCrossoverPipelineV2 extends CrossoverPipeline {
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
        {
           ...
           /* Produces <i>n</i> individuals from the given subpopulation
            ...
            for(int x=0;x<numTries;x++)
            {
                ...
                p1 = nodeselect1.pickNode(state,subpopulation,thread,parents[0],parents[0].trees[t1], true);

                // pick a node in individual 2
                p2 = nodeselect2.pickNode(state,subpopulation,thread,parents[1],parents[1].trees[t2], false);
